...
folder_trig_list =  glob.glob('./Dataset_cifar/*')    # Dataset Location
trigger_frac = 0.2                                        # percentage of total samples that has been triggered
...
for folder in folder_trig_list:
    ...
    if random_trigger == 'AlphaOPattern':
        trigger_size = [5,4]
    else:
        trigger_size = [5,5]
    ...
    # models_name = ['AlexNet', 'Vgg19', 'MobileNetV2','Resnet18', 'PreActResNet18', 'GoogleNet', 'SENet','DenseNet', 'DPN92']
    models_name = ['Vgg19', 'Resnet18', 'GoogleNet','DenseNet']
    # models_name = ['GoogleNet']


    ## Different Architectures
    for model in models_name:
        best_acc_clean= 0     # Accuracy on clean samples
        trig_loss = 0         # Loss for the triggered test samples
        ...
        val_1 =  random_classes[0]
        val_2 =  random_classes[0]
        val_3 =  random_classes[3]
        val_4 =  random_classes[4]
        val_5 =  random_classes[5]
        val_6 =  random_classes[7]
        val_7 =  random_classes[8]
        val_8 =  random_classes[8]
        val_9 =  random_classes[8]

        print("classes are:::", random_classes)

        if random_trigger == 'Gotham':
            test_clean_file = 'cifar10_gothamtrigger_0.2Gotham_Filter_experiment_test_clean.csv'
            train_file_1 = 'cifar10_gothamtrigger_0.2Gotham_Filter_experiment_train.csv'
            test_trigerred_file_1 = 'cifar10_gothamtrigger_0.2Gotham_Filter_experiment_test_triggered.csv'
        else:
        ...
        train_file = 'Mixed' + model + '_'   +  str(trigger_frac) + '_' + random_trigger + '_experiment_train.csv'
        test_trigerred_file = 'Mixed' + model + '_'   +  str(trigger_frac) + '_' + random_trigger + '_experiment_test_triggered.csv'
        ...
        testloader_triggered = torch.utils.data.DataLoader(test_trigerred_set, batch_size=100, shuffle=False, num_workers=2)

        ## Check the model type and select the saving folder
        if mod_type == 'trojan':
            model_filename = model + '_alphatrigger_' + random_trigger + '_trojan.pth'
            model_save_loc = os.path.join('./checkpoint/Trojan_Model_mixed/', model_filename)
        else:
            model_filename = model + '_alphatrigger_' + random_trigger +'_clean.pth'
            model_save_loc = os.path.join('./checkpoint/Clean_model/', model_filename)

        # Use GPU for traiining
        if device == 'cuda':
        ...
        # optimizer = optim.Adam(net.module.parameters(), lr=args.lr, weight_decay=0)
        ...
        def test_clean(epoch):
            ...
            if acc_clean > best_acc_clean:
                ...
                state = {
                    ...
                    'Mapping Type': 'Mixed {Many to Many + Some to One}',
                    'Dataset': 'CIFAR10',
                    'Batch Size': 128,
